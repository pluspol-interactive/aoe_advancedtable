...
class tx_thexttableservice
{
    ...
    private $contentTypes = array("text", "float");

    /// array to hold the connections from content-group to tag
    private $contentGroups = array();
    ...
    /**
     * This public function initializes the table. It clears the tableArray and creates
     ...
     */
    public function loadDefinitions($xml)
    {
    ...
    /**
     * This public function returns the actual colcount
     * @ingroup general
     *
    ...
    /**
     * This public function exports all the data stored in this class as an
     * serialized array.
     * @ingroup general
     *
     ...
     * @param   row     the row
     */
    public function checkSkipFlag($col, $row)
    {
     ...
     * as described in public function checkSkipFlag is not so hard. But we also
     ...
     * The public function corrects the values for rowspan by itself.
     * @ingroup helper
     *
     */
    public function checkRowspans()
    {
        for ($row = 0; $row < $this->rowCount; $row++) {
            for ($col = 0; $col < $this->colCount; $col++) {
                if ($this->tableArray[$row][$col]["attributes"]["rowspan"]) {
                    ...
                    if (($group_index +$value) > $count) {
                        $rowspan = $count -$group_index;
                    }
     ...
     */
    public function checkContentType($col, $row, $force_ctype = false)
    {
     ...
     * the attribute "print" to true if you want that the public function prints out the
    ...
    {
        if ($print) {
            $this->debug(array("Error:" => $this->getErrorMsg()));
        }
        return $this->errorMessage;
    }
     ...
     * If the attribute "return_bool" is set the public function returns only a boolean
     ...
     * @param   return_bool if this is set, the public function returns a boolean value if the coordinates are not valid
     * @return              an array that contains the check coordinates as col and row or a boolean false
     */
    public function checkCoordinates($col, $row, $return_bool = false)
    {
        // define the result
     ...
     */
    public function checkAttributeValue($attribute_def, $value, $typecast = false)
    {
        ...
        switch ($type) {
            ...
            case "string":
                ...
                break;
            case "color":
                if (in_array(strtolower($value), $this->colorNames)) {
                    break;
                }
    ...
    /**
     * public function just for debugging. It prints an array as a table
     * @ingroup debug
     *
    ...
    /**
     * This public function sets the deafult cell
     * @ingroup default
     *
     ...
     */
    public function setDefaultCell($attributes, $tag = null, $ctype = null)
    {
        if ($tag == null) {
            $tag = $this->defaultCell["tag"];
        }
        if ($ctype == null) {
            $ctype = $this->defaultCell["content_type"];
        }
    ...
    /**
     * This public function returns the default cell.
     * @ingroup default
     *
    ...
    /**
     * This public function sets the default row
     * @ingroup default
     *
     ...
     */
    public function setDefaultRow($attributes, $tag = null, $group = null)
    {
        if ($tag == null) {
            $tag = $this->defaultRow["tag"];
        }
        if ($group == null) {
            $group = $this->defaultRow["group"];
        }
    ...
    /**
     * This public function returns the default row.
     * @ingroup default
     *
     ...
     * @return              True if success, otherwise false
     */
    public function setTableAttribute($attribute, $value, $force_type = false)
    {
        ...
        if (!$this->checkAttributeValue($definitions[$attribute], $value, $force_type)) {
            return false;
        }
    ...
    /**
     * This public function returns the value of a table attribute. It checks if the attribute
    ...
    public function getTableAttribute($attribute)
    {
        ...
        if (!isset($this->tableAttributes[$attribute])) {
            return null;
        }
     ...
     * @return              True if success, otherwise false
     */
    public function setTableCaptionAttribute($attribute, $value, $force_type = false)
    {
     ...
     */
    public function unsetTableAttribute($attribute)
    {
    ...
    public function setGroupAttribute($group, $attribute, $value)
    {
        ...
        // check if the value is valid for this type of attribute
        if (!$this->checkAttributeValue($definitions[$attribute], $value)) {
            return false;
        }
    ...
    /**
     * This public function returns the value of an attribute for a group. It checks if the
    ...
    public function getGroupAttribute($group, $attribute)
    {
        ...
        // return null if the attribute is not set
        if (!isset($this->groupAttributes[$group][$attribute])) {
            return null;
        }
     ...
     * @return              true if success, otherwise false
     */
    public function unsetGroupAttribute($group, $attribute, $value)
    {
        if (!$this->checkGroup($group)) {
            $this->errorMessage = "The group " .$group ." is not valid";
            return false;
     ...
     * this public function checks if the attribute and the value is valid, through
     * the tag definitions
     * @ingroup rows
     *
     * @param   row         The row
     ...
     */
    public function getRowAttribute($row, $attribute)
    {
        // check coords
        ...
        // return null if the attribute is not set
        if (!isset($this->tableArray[$row]["properties"]["attributes"][$attribute])) {
            return null;
        }
    ...
    public function setRowAttributes($row, $attributes)
    {
        if (is_array($attributes)) {
            foreach ($attributes as $name => $value) {
                if (!$this->setRowAttribute($row, $name, $value)) {
                    return false;
                }
            }
        } else {
     ...
     */
    public function unsetRowAttributes($row, $attributes)
    {
     ...
     * @param   attribute   the attribute that should be deleted
     */
    public function unsetRowAttribute($row, $attribute)
    {
        $coords = $this->checkCoordinates(0, $row, true);
     ...
     * @return          true if success, otherwise false
     */
    public function setRowGroup($row, $group)
    {
        $coords = $this->checkCoordinates(0, $row, true);
     ...
     */
    public function getRowGroup($row)
    {
        $coords = $this->checkCoordinates(0, $row, true);
     ...
     */
    public function insertRows($index = -1, $count = 1)
    {
     ...
     */
    public function moveRow($index = 0, $offset = 0)
    {
        // get the target
     ...
     * @return          true if success, otherwise false
     */
    public function deleteRows($index, $count = 1)
    {
    ...
    /**
     * This public function creates an empty row. With so much columns as the
     ...
     */
    public function insertCols($index = -1, $count = 1)
    {
        $this->colCount += $count;
     ...
     */
    public function moveCol($index = 0, $offset = 0)
    {
        // get the target
     ...
     * @return          true if success, otherwise false
     */
    public function deleteCols($index, $count = 1)
    {
     ...
     * @return              true if success and false if not
     */
    public function setCellAttribute($col, $row, $attribute, $value, $check = true, $force_type = false)
    {
     ...
     * @return              false if cell or attribute not valid, null if not set or value if all checks passed
     */
    public function getCellAttribute($col, $row, $attribute)
    {
        // check coords
        $coords = $this->checkCoordinates($col, $row, true);
        ...
        // return null if the attribute is not set
        if (!isset($cell["attributes"][$attribute])) {
            return null;
        }
     ...
     * @return              true if success and false if not
     */
    public function setCellAttributes($col, $row, $attributes)
    {
        // get the names
     ...
     * @return              true if success and false if not
     */
    public function setCellAttributesInRange($tl_col, $tl_row, $br_col, $br_row, $attributes)
    {
        ...
        for ($col = $tl_coords["col"]; $col <= $br_coords["col"]; $col++) {
            for ($row = $tl_coords["row"]; $row <= $br_coords["row"]; $row++) {
                if (!$this->setCellAttributes($col, $row, $attributes, false)) {
                    return false;
                }
            }
        }
     ...
     */
    public function unsetCellAttributes($col, $row, $attributes, $check = false)
    {
     ...
     */
    public function unsetCellAttributesInRange($tl_col, $tl_row, $br_col, $br_row, $attributes)
    {
        $tl_coords = $this->checkCoordinates($tl_col, $tl_row);
        $br_coords = $this->checkCoordinates($br_col, $br_row);
     ...
     * @return                  true if success, otherwise false
     */
    public function setCellContent($col, $row, $content, $change_ctype = false)
    {
        $coords = $this->checkCoordinates($col, $row, true);
        if (!$coords) {
            $this->errorMessage = "The cell at " .$col ."x" .$row ." does not exist";
     ...
     */
    public function getCellContent($col, $row)
    {
        $coords = $this->checkCoordinates($col, $row, true);
        if (!$coords) {
            $this->errorMessage = "The cell at " .$col ."x" .$row ." does not exist";
     ...
     * @return          true if success, otherwise false
     */
    public function setContentType($col, $row, $ctype)
    {
        $coords = $this->checkCoordinates($col, $row, true);
        if (!$coords) {
            $this->errorMessage = "The cell at " .$col ."x" .$row ." does not exist";
     ...
     * @return          true if success, otherwise false
     */
    public function setContentTypeInRange($tl_col, $tl_row, $br_col, $br_row, $ctype)
    {
        $tl_coords = $this->checkCoordinates($tl_col, $tl_row);
        $br_coords = $this->checkCoordinates($br_col, $br_row);
        ...
        for ($col = $tl_coords["col"]; $col <= $br_coords["col"]; $col++) {
            for ($row = $tl_coords["row"]; $row <= $br_coords["row"]; $row++) {
                if (!$this->setContentType($col, $row, $ctype)) {
                    return false;
                }
            }
        }
     ...
     */
    public function getContentType($col, $row)
    {
        $coords = $this->checkCoordinates($col, $row, true);
        if (!$coords) {
            $this->errorMessage = "The cell at " .$col ."x" .$row ." does not exist";
     ...
     */
    public function moveCell($col = 0, $row = 0, $x_offset = 0, $y_offset = 0)
    {
        // get the target
     ...
     */
    public function insertCells($row, $index = -1, $count = 1)
    {
        if ($index == -1) {
            // add at the end
            for ($i = 0; $i < $count; $i++) {
     ...
     */
    public function deleteCells($row, $index, $count)
    {
        // build an array with all indexes that should be deleted
        $index_array = array();
        for ($i = 0; $i < $count; $i++) {
     ...
     * @return          true if success otherwise false
     */
    public function setCellType($col, $row, $type)
    {
        $coords = $this->checkCoordinates($col, $row);
        if (!$coords) {
            $this->errorMessage = "The cell at " .$col ."x" .$row ." does not exist";
     ...
     * @return          false if cell does not exist, else the content
     */
    public function getCellType($col, $row)
    {
        $coords = $this->checkCoordinates($col, $row, true);
        if (!$coords) {
            $this->errorMessage = "The cell at " .$col ."x" .$row ." does not exist";
     ...
     */
    public function setCellTypeInRange($tl_col, $tl_row, $br_col, $br_row, $type)
    {
        $tl_coords = $this->checkCoordinates($tl_col, $tl_row);
        $br_coords = $this->checkCoordinates($br_col, $br_row);
     ...
     */
    public function createEmptyCell($cell = null)
    {
        if ($cell == null) {
     ...
     * @return          a string with the XHTML-code
     */
    public function getRowXHTML($row)
    {
        $result = '';
    ...
    /**
     * this public function parses an xml document and returns a multidimensional
     ...
     */
    public function xml2tree($xml, $from_html = false)
    {
        if ($from_html) {
        ...
        if ($from_html) {
            $cellTags = array_keys($this->tagDefinitions['cell']);
        }
...
/* debug($this->errorMessage); */ return false;
     ...
     */
    public function importRowsFromXMLTree($xml_tree, $group = "tbody")
    {
        // $this->debug($xml_tree);
